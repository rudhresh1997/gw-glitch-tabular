...
def objective(trial):
    ...
    attn_embed_dim, num_heads = trial.suggest_categorical("embed_dim_head_pair", valid_pairs)
    ...
    attention_pooling = trial.suggest_categorical("attention_pooling", [True, False])
...
best_params["attn_embed_dim"] = best_embed_dim
...
os.makedirs("./img_autoint", exist_ok=True)

pd.DataFrame(conf_mat, index=class_labels, columns=class_labels).to_csv("./img_autoint/confusion_matrix_full.csv")
pd.DataFrame({"class": class_labels, "f1_score": per_class_f1}).to_csv("./img_autoint/per_class_f1_full.csv", index=False)
...
metrics_df.to_csv("./img_autoint/metrics.csv", index=False)
...
plt.legend()
plt.savefig("./img_autoint/roc_curve.png")
...
plt.legend()
plt.savefig("./img_autoint/pr_curve.png")
...
ax.set_title("Confusion matrix for AutoInt (row-normalized)", pad=14)
...
plt.tight_layout()
plt.savefig("./img_autoint/conf_matrix_row_normalized_PRD.png", dpi=600, bbox_inches="tight")
plt.savefig("./img_autoint/conf_matrix_row_normalized_PRD.pdf", bbox_inches="tight")
plt.close()
...
ax.set_title("Confusion matrix for AutoInt (raw counts)", pad=14)
ax.set_xlabel("Predicted class")
ax.set_ylabel("True class")
plt.xticks(rotation=45, ha="right")
plt.yticks(rotation=0)
...
plt.tight_layout()
plt.savefig("./img_autoint/conf_matrix_counts_PRD.png", dpi=600, bbox_inches="tight")
plt.savefig("./img_autoint/conf_matrix_counts_PRD.pdf", bbox_inches="tight")
plt.close()
...
# === Export clean AutoInt inference model, run Captum, save CSVs, and compare to TreeSHAP ===
...
class _LightningWrapper(nn.Module):
    """
    Wrapper for AutoInt LightningModule.
    Prepares dict input {continuous, categorical} as AutoInt expects.
    """
...
# --- CHANGE: Force LightningWrapper for AutoInt ---
def build_exported_model(tabular_model, scaler, device="cpu"):
    lm = tabular_model.model
    mu = scaler.mean_
    sigma = scaler.scale_
    print("[ExportedAutoInt] Using LightningWrapper with dict input")
...
with open("./img_autoint/attr_metadata_autoint.json", "w") as f:
    json.dump({
        "model": "AutoInt",
        "device": device,
...
per_class_path = "./img_autoint/captum_ig_per_class.csv"
global_path    = "./img_autoint/captum_ig_global.csv"
...
autoint_g = pd.read_csv(global_path).set_index("feature")
common_feats = xgb_g.index.intersection(autoint_g.index)

rho_g, p_g = spearmanr(
    xgb_g.loc[common_feats, "mean_abs_shap"].rank(ascending=False),
    autoint_g.loc[common_feats, "mean_abs_attr"].rank(ascending=False),
)
print(f"[GLOBAL] Spearman ρ(TreeSHAP vs Captum IG, AutoInt) = {rho_g:.3f} (p={p_g:.3g})")

try:
    xgb_c = pd.read_csv(xgb_per_class_csv)
    autoint_c = pd.read_csv(per_class_path)
    classes_to_check = sorted(set(xgb_c["class"]).intersection(set(autoint_c["class"])))
    for cname in classes_to_check:
        xx = xgb_c[xgb_c["class"] == cname].set_index("feature")
        mm = autoint_c[autoint_c["class"] == cname].set_index("feature")
...
# === Export Correlation Results to CSV (AutoInt) ===
corr_rows = []
...
out_path = "./img_autoint/captum_vs_treeshap_corr.csv"
