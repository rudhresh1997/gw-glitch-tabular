...
def objective(trial):
    num_attn_blocks = trial.suggest_int("num_attn_blocks", 3, 9, step=3)
    transformer_head_dim = trial.suggest_categorical("transformer_head_dim", [32, 64, 128, 256])
    attn_dropout = trial.suggest_float("attn_dropout", 0.05, 0.3)
    add_norm_dropout = trial.suggest_float("add_norm_dropout", 0.05, 0.3)
    ff_dropout = trial.suggest_float("ff_dropout", 0.05, 0.3)
    ff_hidden_multiplier = trial.suggest_int("ff_hidden_multiplier", 2, 6)
    transformer_activation = trial.suggest_categorical("transformer_activation", ["ReLU", "LeakyReLU", "GEGLU", "SwiGLU"])
...
os.makedirs("./img_fttransformer", exist_ok=True)

pd.DataFrame(conf_mat, index=class_labels, columns=class_labels).to_csv("./img_fttransformer/confusion_matrix_full.csv")
pd.DataFrame({"class": class_labels, "f1_score": per_class_f1}).to_csv("./img_fttransformer/per_class_f1_full.csv", index=False)
...
metrics_df.to_csv("./img_fttransformer/metrics.csv", index=False)
...
plt.legend()
plt.savefig("./img_fttransformer/roc_curve.png")
...
plt.legend()
plt.savefig("./img_fttransformer/pr_curve.png")
...
ax.set_title("Confusion matrix for FT-Transformer (row-normalized)", pad=14)
...
plt.tight_layout()
plt.savefig("./img_fttransformer/conf_matrix_row_normalized_PRD.png", dpi=600, bbox_inches="tight")
plt.savefig("./img_fttransformer/conf_matrix_row_normalized_PRD.pdf", bbox_inches="tight")
plt.close()
...
ax.set_title("Confusion matrix for FT-Transformer (raw counts)", pad=14)
ax.set_xlabel("Predicted class")
ax.set_ylabel("True class")
plt.xticks(rotation=45, ha="right")
plt.yticks(rotation=0)
...
plt.tight_layout()
plt.savefig("./img_fttransformer/conf_matrix_counts_PRD.png", dpi=600, bbox_inches="tight")
plt.savefig("./img_fttransformer/conf_matrix_counts_PRD.pdf", bbox_inches="tight")
plt.close()




# === Export clean FT-Transformer inference model, run Captum, save CSVs, and compare to TreeSHAP ===
...
class _LightningWrapper(nn.Module):
    """
    Wrapper for FT-Transformer LightningModule.
    Prepares dict input {continuous, categorical} as FT-Transformer expects.
    """
...
# --- CHANGE: Force LightningWrapper for FT-Transformer ---
def build_exported_model(tabular_model, scaler, device="cpu"):
    lm = tabular_model.model
    mu = scaler.mean_
    sigma = scaler.scale_
    print("[ExportedFTTransformer] Using LightningWrapper with dict input")
...
with open("./img_fttransformer/attr_metadata_fttransformer.json", "w") as f:
    json.dump({
...
per_class_path = "./img_fttransformer/captum_ig_per_class.csv"
global_path    = "./img_fttransformer/captum_ig_global.csv"
...
fttransformer_g = pd.read_csv(global_path).set_index("feature")
common_feats = xgb_g.index.intersection(fttransformer_g.index)

rho_g, p_g = spearmanr(
    xgb_g.loc[common_feats, "mean_abs_shap"].rank(ascending=False),
    fttransformer_g.loc[common_feats, "mean_abs_attr"].rank(ascending=False),
)
print(f"[GLOBAL] Spearman ρ(TreeSHAP vs Captum IG, FT-Transformer) = {rho_g:.3f} (p={p_g:.3g})")

try:
    xgb_c = pd.read_csv(xgb_per_class_csv)
    fttransformer_c = pd.read_csv(per_class_path)
    classes_to_check = sorted(set(xgb_c["class"]).intersection(set(fttransformer_c["class"])))
    for cname in classes_to_check:
        xx = xgb_c[xgb_c["class"] == cname].set_index("feature")
        mm = fttransformer_c[fttransformer_c["class"] == cname].set_index("feature")
...
# === Export Correlation Results to CSV (FT-Transformer) ===
corr_rows = []
...
out_path = "./img_fttransformer/captum_vs_treeshap_corr.csv"
