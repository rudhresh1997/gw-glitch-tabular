...
def objective(trial):
    ...
    gflu_feature_init_sparsity = trial.suggest_float("gflu_feature_init_sparsity", 0.2, 0.5)
...
os.makedirs("./img_gandalf", exist_ok=True)

pd.DataFrame(conf_mat, index=class_labels, columns=class_labels).to_csv("./img_gandalf/confusion_matrix_full.csv")
pd.DataFrame({"class": class_labels, "f1_score": per_class_f1}).to_csv("./img_gandalf/per_class_f1_full.csv", index=False)
...
metrics_df.to_csv("./img_gandalf/metrics.csv", index=False)
...
plt.legend()
plt.savefig("./img_gandalf/roc_curve.png")
...
plt.legend()
plt.savefig("./img_gandalf/pr_curve.png")
...
ax.set_title("Confusion matrix for GANDALF (row-normalized)", pad=14)
...
plt.tight_layout()
plt.savefig("./img_gandalf/conf_matrix_row_normalized_PRD.png", dpi=600, bbox_inches="tight")
plt.savefig("./img_gandalf/conf_matrix_row_normalized_PRD.pdf", bbox_inches="tight")
plt.close()
...
ax.set_title("Confusion matrix for GANDALF (raw counts)", pad=14)
ax.set_xlabel("Predicted class")
ax.set_ylabel("True class")
plt.xticks(rotation=45, ha="right")
plt.yticks(rotation=0)
...
plt.tight_layout()
plt.savefig("./img_gandalf/conf_matrix_counts_PRD.png", dpi=600, bbox_inches="tight")
plt.savefig("./img_gandalf/conf_matrix_counts_PRD.pdf", bbox_inches="tight")
plt.close()
...
# === Export clean GANDALF inference model, run Captum, save CSVs, and compare to TreeSHAP ===
...
class _LightningWrapper(nn.Module):
    """
    Wrapper for GANDALF LightningModule.
    Prepares dict input {continuous, categorical} as GANDALF expects.
    """
...
# --- CHANGE: Force LightningWrapper for GANDALF ---
def build_exported_model(tabular_model, scaler, device="cpu"):
    lm = tabular_model.model
    mu = scaler.mean_
    sigma = scaler.scale_
    print("[ExportedGANDALF] Using LightningWrapper with dict input")
...
with open("./img_gandalf/attr_metadata_gandalf.json", "w") as f:
    json.dump({
        "model": "GANDALF",
        "device": device,
...
per_class_path = "./img_gandalf/captum_ig_per_class.csv"
global_path    = "./img_gandalf/captum_ig_global.csv"
...
gandalf_g = pd.read_csv(global_path).set_index("feature")
common_feats = xgb_g.index.intersection(gandalf_g.index)

rho_g, p_g = spearmanr(
    xgb_g.loc[common_feats, "mean_abs_shap"].rank(ascending=False),
    gandalf_g.loc[common_feats, "mean_abs_attr"].rank(ascending=False),
)
print(f"[GLOBAL] Spearman ρ(TreeSHAP vs Captum IG, GANDALF) = {rho_g:.3f} (p={p_g:.3g})")

try:
    xgb_c = pd.read_csv(xgb_per_class_csv)
    gandalf_c = pd.read_csv(per_class_path)
    classes_to_check = sorted(set(xgb_c["class"]).intersection(set(gandalf_c["class"])))
    for cname in classes_to_check:
        xx = xgb_c[xgb_c["class"] == cname].set_index("feature")
        mm = gandalf_c[gandalf_c["class"] == cname].set_index("feature")
...
# === Export Correlation Results to CSV (GANDALF) ===
corr_rows = []
...
out_path = "./img_gandalf/captum_vs_treeshap_corr.csv"
