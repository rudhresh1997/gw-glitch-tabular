...
def objective(trial):
    ...
    feature_mask_function = trial.suggest_categorical("feature_mask_function", ["entmax", "sparsemax", "softmax"])
    tree_dropout = trial.suggest_float("tree_dropout", 0.0, 0.2)
    tree_wise_attention = use_attention
    tree_wise_attention_dropout = trial.suggest_float("tree_wise_attention_dropout", 0.0, 0.2) if use_attention else 0.0
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(df, df[target_col])):
        ...
        trainer_config = TrainerConfig(
            batch_size=512,             # Fewer for Optuna Search
            max_epochs=8,               # Fewer for Optuna Search
            early_stopping_patience=2,  # Fewer for Optuna Search
            accelerator="gpu",          # force GPU
            devices=1                   # single GPU per trial
        )
...
plt.ylabel("Actual")
plt.savefig("./img_gate/conf_matrix.png")
