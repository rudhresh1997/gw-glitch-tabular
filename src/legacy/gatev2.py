...
train_df, test_df = train_test_split(
    df, test_size=0.36, random_state=42, stratify=df["ml_label"]
)
val_df, test_df = train_test_split(
    test_df, test_size=20 / 36, random_state=42, stratify=test_df["ml_label"]
)

target_col = ["ml_label"]
num_features = [
    col for col in df.columns if df[col].dtype in ["int64", "float64"] and col != target_col
]
...
def objective(trial):
    use_attention = trial.suggest_categorical("tree_wise_attention", [True, False])

    gflu_stages = trial.suggest_categorical("gflu_stages", [4, 6, 8])
    gflu_dropout = trial.suggest_float("gflu_dropout", 0.0, 0.3)
    tree_depth = trial.suggest_categorical("tree_depth", [3, 4, 5])
    num_trees = trial.suggest_categorical("num_trees", [10, 20, 40])
    binning_activation = trial.suggest_categorical("binning_activation", ["entmoid", "sparsemoid"])
    feature_mask_function = trial.suggest_categorical(
        "feature_mask_function", ["entmax", "sparsemax", "softmax"]
    )
    tree_dropout = trial.suggest_float("tree_dropout", 0.0, 0.2)
    tree_wise_attention = use_attention
    tree_wise_attention_dropout = (
        trial.suggest_float("tree_wise_attention_dropout", 0.0, 0.2)
        if use_attention
        else 0.0
    )
...
target_col = ["ml_label"]
num_features = [
    col for col in df.columns if df[col].dtype in ["int64", "float64"] and col not in target_col
]
...
y_test_pred_prob_full = pred_df_full[
    [col for col in pred_df_full.columns if "_proba" in col]
].values
...
os.makedirs("./img_gate", exist_ok=True)

pd.DataFrame(conf_mat, index=class_labels, columns=class_labels).to_csv(
    "./img_gate/confusion_matrix_full.csv"
)
pd.DataFrame({"class": class_labels, "f1_score": per_class_f1}).to_csv(
    "./img_gate/per_class_f1_full.csv", index=False
)
...
metrics_df.to_csv("./img_gate/metrics.csv", index=False)


# Plot ROC Curve
plt.figure()
for i, cls in enumerate(class_labels):
    fpr, tpr, _ = roc_curve(y_test_full == cls, y_test_pred_prob_full[:, i])
    plt.plot(fpr, tpr, label=f"Class {cls} (AUC = {auc(fpr, tpr):.2f})")
plt.xlabel("False Positive Rate")
plt.ylabel("True Positive Rate")
plt.legend()
plt.savefig("./img_gate/roc_curve.png")

# Plot Precision-Recall Curve
plt.figure()
for i, cls in enumerate(class_labels):
    precision, recall, _ = precision_recall_curve(y_test_full == cls, y_test_pred_prob_full[:, i])
    plt.plot(recall, precision, label=f"Class {cls}")
plt.xlabel("Recall")
plt.ylabel("Precision")
plt.legend()
plt.savefig("./img_gate/pr_curve.png")
...
ax.set_title("Confusion matrix for GATE (row-normalized)", pad=14)
...
plt.tight_layout()
plt.savefig("./img_gate/conf_matrix_row_normalized_PRD.png", dpi=600, bbox_inches="tight")
plt.savefig("./img_gate/conf_matrix_row_normalized_PRD.pdf", bbox_inches="tight")
plt.close()
...
ax.set_title("Confusion matrix for GATE (raw counts)", pad=14)
ax.set_xlabel("Predicted class")
ax.set_ylabel("True class")
plt.xticks(rotation=45, ha="right")
plt.yticks(rotation=0)
...
plt.tight_layout()
plt.savefig("./img_gate/conf_matrix_counts_PRD.png", dpi=600, bbox_inches="tight")
plt.savefig("./img_gate/conf_matrix_counts_PRD.pdf", bbox_inches="tight")
plt.close()


# === Export clean GATE inference model, run Captum, save CSVs, and compare to TreeSHAP ===
...
class _LightningWrapper(nn.Module):
    """
    Wrapper for GATE LightningModule.
    Prepares dict input {continuous, categorical} as GATE expects.
    """
...
# --- CHANGE: Force LightningWrapper for GATE ---
def build_exported_model(tabular_model, scaler, device="cpu"):
    lm = tabular_model.model
    mu = scaler.mean_
    sigma = scaler.scale_
    print("[ExportedGATE] Using LightningWrapper with dict input")
...
with open("./img_gate/attr_metadata_gate.json", "w") as f:
    json.dump(
        {
            "model": "GATE",
            "device": device,
...
per_class_df["rank_within_class"] = per_class_df.groupby("class")["mean_abs_attr"].rank(
...
per_class_path = "./img_gate/captum_ig_per_class.csv"
global_path = "./img_gate/captum_ig_global.csv"
...
gate_g = pd.read_csv(global_path).set_index("feature")
common_feats = xgb_g.index.intersection(gate_g.index)

rho_g, p_g = spearmanr(
    xgb_g.loc[common_feats, "mean_abs_shap"].rank(ascending=False),
    gate_g.loc[common_feats, "mean_abs_attr"].rank(ascending=False),
)
print(f"[GLOBAL] Spearman ρ(TreeSHAP vs Captum IG, GATE) = {rho_g:.3f} (p={p_g:.3g})")

try:
    xgb_c = pd.read_csv(xgb_per_class_csv)
    gate_c = pd.read_csv(per_class_path)
    classes_to_check = sorted(set(xgb_c["class"]).intersection(set(gate_c["class"])))
    for cname in classes_to_check:
        xx = xgb_c[xgb_c["class"] == cname].set_index("feature")
        mm = gate_c[gate_c["class"] == cname].set_index("feature")
...
# === Export Correlation Results to CSV (GATE) ===
corr_rows = []
...
out_path = "./img_gate/captum_vs_treeshap_corr.csv"
