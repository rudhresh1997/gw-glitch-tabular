...
def objective(trial):
    ...
    num_trees = trial.suggest_categorical("num_trees", [256, 512, 1024])
    depth = trial.suggest_int("depth", 3, 6)
    additional_tree_output_dim = trial.suggest_categorical("additional_tree_output_dim", [1, 3])
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(df, y_all)):
        ...
        trainer_config = TrainerConfig(
            ...
            checkpoints_path=SEARCH_CKPT_DIR,
            load_best=False
        )
...
pd.DataFrame(conf_mat, index=class_labels, columns=class_labels).to_csv("./img_node/confusion_matrix_full.csv")
pd.DataFrame({"class": class_labels, "f1_score": per_class_f1}).to_csv("./img_node/per_class_f1_full.csv", index=False)
...
ax.set_title("Confusion matrix for NODE (row-normalized)", pad=14)
...
plt.tight_layout()
plt.savefig("./img_node/conf_matrix_row_normalized_PRD.png", dpi=600, bbox_inches="tight")
plt.savefig("./img_node/conf_matrix_row_normalized_PRD.pdf", bbox_inches="tight")
plt.close()
...
ax.set_title("Confusion matrix for NODE (raw counts)", pad=14)
ax.set_xlabel("Predicted class")
ax.set_ylabel("True class")
plt.xticks(rotation=45, ha="right")
plt.yticks(rotation=0)
...
plt.tight_layout()
plt.savefig("./img_node/conf_matrix_counts_PRD.png", dpi=600, bbox_inches="tight")
plt.savefig("./img_node/conf_matrix_counts_PRD.pdf", bbox_inches="tight")
plt.close()

# === Export clean NODE inference model, run Captum, save CSVs, and compare to TreeSHAP ===
import json
import numpy as np
import pandas as pd
import torch
...
# --- CHANGE: Force LightningWrapper for NODE ---
...
# === Export Correlation Results to CSV (NODE) ===
corr_rows = []
