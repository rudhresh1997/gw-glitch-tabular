...
def objective(trial):
    ...
    gamma = trial.suggest_categorical("gamma", [1.0, 1.5, 2.0])
...
os.makedirs("./img_tabnet", exist_ok=True)

pd.DataFrame(conf_mat, index=class_labels, columns=class_labels).to_csv("./img_tabnet/confusion_matrix_full.csv")
pd.DataFrame({"class": class_labels, "f1_score": per_class_f1}).to_csv("./img_tabnet/per_class_f1_full.csv", index=False)
...
metrics_df.to_csv("./img_tabnet/metrics.csv", index=False)
...
plt.legend()
plt.savefig("./img_tabnet/roc_curve.png")
...
plt.legend()
plt.savefig("./img_tabnet/pr_curve.png")
...
ax.set_title("Confusion matrix for TabNet (row-normalized)", pad=14)
...
plt.tight_layout()
plt.savefig("./img_tabnet/conf_matrix_row_normalized_PRD.png", dpi=600, bbox_inches="tight")
plt.savefig("./img_tabnet/conf_matrix_row_normalized_PRD.pdf", bbox_inches="tight")
plt.close()
...
ax.set_title("Confusion matrix for TabNet (raw counts)", pad=14)
ax.set_xlabel("Predicted class")
ax.set_ylabel("True class")
plt.xticks(rotation=45, ha="right")
plt.yticks(rotation=0)
...
plt.tight_layout()
plt.savefig("./img_tabnet/conf_matrix_counts_PRD.png", dpi=600, bbox_inches="tight")
plt.savefig("./img_tabnet/conf_matrix_counts_PRD.pdf", bbox_inches="tight")
plt.close()


# === Export clean TabNet inference model, run Captum, save CSVs, and compare to TreeSHAP ===
import json
import numpy as np
import pandas as pd
import torch
...
class _LightningWrapper(nn.Module):
    """
    Wrapper for TabNet LightningModule.
    Prepares dict input {continuous, categorical} as TabNet expects.
    """
...
# --- CHANGE: Force LightningWrapper for TabNet ---
def build_exported_model(tabular_model, scaler, device="cpu"):
    lm = tabular_model.model
    mu = scaler.mean_
    sigma = scaler.scale_
    print("[ExportedTabNet] Using LightningWrapper with dict input")
...
with open("./img_tabnet/attr_metadata_tabnet.json", "w") as f:
    json.dump({
        "model": "TabNet",
        "device": device,
...
per_class_path = "./img_tabnet/captum_ig_per_class.csv"
global_path = "./img_tabnet/captum_ig_global.csv"
...
tabnet_g = pd.read_csv(global_path).set_index("feature")
common_feats = xgb_g.index.intersection(tabnet_g.index)

rho_g, p_g = spearmanr(
    xgb_g.loc[common_feats, "mean_abs_shap"].rank(ascending=False),
    tabnet_g.loc[common_feats, "mean_abs_attr"].rank(ascending=False),
)
print(f"[GLOBAL] Spearman ρ(TreeSHAP vs Captum IG, TabNet) = {rho_g:.3f} (p={p_g:.3g})")

try:
    xgb_c = pd.read_csv(xgb_per_class_csv)
    tabnet_c = pd.read_csv(per_class_path)
    classes_to_check = sorted(set(xgb_c["class"]).intersection(set(tabnet_c["class"])))
    for cname in classes_to_check:
        xx = xgb_c[xgb_c["class"] == cname].set_index("feature")
        mm = tabnet_c[tabnet_c["class"] == cname].set_index("feature")
...
# === Export Correlation Results to CSV (TabNet) ===
corr_rows = []
...
out_path = "./img_tabnet/captum_vs_treeshap_corr.csv"
